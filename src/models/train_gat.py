...
def load_and_featurize_graphs(file_path: str, featurizer: dc.feat.Featurizer):
    ...
    y_valid = df['pIC50'].values[valid_indices]
...
def train_gat():
    ...
    # Initialize the featurizer (edges include bond features; key for GAT)
    ...
    # Initialize GAT model (baseline NN variables, tuning could yield better results)
    logger.info("Initializing PyTorch Graph Attention Network (GAT)...")
    ...
    logger.info("Training GAT with early stopping (patience=10)...")
    ...
    logger.info(f"Saved GAT model to {model_out_dir}")
