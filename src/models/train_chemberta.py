...
def tune_chemberta():
    ...
    predictions_path = os.path.join(results_out_dir, "chemberta_test_predictions.csv")
    test_df.to_csv(predictions_path, index=False)
    logger.info(f"Saved raw test predictions to {predictions_path}")
    ...
    logger.info(f"Saved metrics to {results_out_dir}")
